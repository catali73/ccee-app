  ...
  return (
    <header style={{background:'#fff',borderBottom:'1px solid #e4e4e7',position:'sticky',top:0,zIndex:100}}>
      <div style={{maxWidth:1100,margin:'0 auto',padding:'0 16px',height:56,display:'flex',alignItems:'center',gap:12}}>
        ...
        {isMobile?(
          <>
            <span style={{fontSize:12,fontWeight:500,color:'#09090b',flex:1,textAlign:'center'}}>{tabs.find(t=>t.id===view)?.label||''}</span>
            <button onClick={()=>setMenuOpen(o=>!o)}
              style={{width:36,height:36,borderRadius:8,border:'1px solid #e4e4e7',background:'#fff',cursor:'pointer',fontSize:16,display:'flex',alignItems:'center',justifyContent:'center'}}>
              {menuOpen?'✕':'☰'}
            </button>
            <BtnO onClick={onLogout} style={{height:28,fontSize:12,padding:'0 8px'}}>Salir</BtnO>
          </>
        ):(
          <>
      ...
      {isMobile&&menuOpen&&(
        <div style={{background:'#fff',borderTop:'1px solid #e4e4e7',padding:'8px 0',boxShadow:'0 4px 12px rgba(0,0,0,0.08)'}}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>handleNav(t.id)}
              style={{display:'flex',alignItems:'center',gap:10,width:'100%',padding:'12px 20px',border:'none',background:view===t.id?'#f4f4f5':'transparent',cursor:'pointer',fontSize:14,fontWeight:view===t.id?600:400,color:view===t.id?'#09090b':'#52525b',textAlign:'left'}}>
              <span>{t.icon}</span>{t.label}
            </button>
          ))}
          <div style={{margin:'8px 16px 4px',borderTop:'1px solid #e4e4e7',paddingTop:8,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <span style={{fontSize:12,color:'#71717a'}}>{user.name}</span>
          </div>
        </div>
      )}
    </header>
            ...
            <div key={s.id}
              ...
              <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
                ...
                <button onClick={e=>{e.stopPropagation();downloadHojaPDF(s.id,s.encuentro);}}
                  ...
                  style={{border:'1px solid #e4e4e7',borderRadius:6,background:'#fafafa',padding:'5px 10px',fontSize:14,cursor:'pointer',color:'#52525b',lineHeight:1,flexShrink:0}}>📄</button>
                <button onClick={e=>{e.stopPropagation();onEditServicio(s.id);}}
                  style={{border:'1px solid #e4e4e7',borderRadius:6,background:'#fafafa',padding:'5px 10px',fontSize:14,cursor:'pointer',color:'#71717a',lineHeight:1,flexShrink:0}}>✏️</button>
                ...
                {editingId===v.id ? <FormRow /> : (
                  <div style={{border:'1px solid #e4e4e7',borderRadius:8,padding:'10px 12px',background:'#fff',display:'flex',alignItems:'center',gap:8}}>
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:13,fontWeight:500}}>{v.articulo}</div>
